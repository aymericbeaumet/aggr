...
<div class="meta">
  {#if metadata.source_display}<span class="meta-field"><a class="domain" href={facetURL(base, 'source', metadata.source_slug || '')} title={metadata.source_title || metadata.source_display}><span class="source-resolved">{metadata.source_display}</span>{#if metadata.is_aggregated}{' '}<em>via {metadata.feed_display}</em>{/if}</a></span>{/if}
