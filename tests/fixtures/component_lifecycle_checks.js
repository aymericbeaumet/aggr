...
(async () => {
  ...
  try {
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      check(Math.abs(artworkBounds.height - panelBounds.height) <= 1, 'podcast artwork fills the entire control-panel height');
      check(Math.abs(panelBounds.width / artworkBounds.width - 1.618) < .02, 'artwork and controls use golden-ratio columns');
